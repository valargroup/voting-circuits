...
- **"What diversifier index do dummy notes use, and why?"** — The builder picks `fvk.address_at(PADDING_DIVERSIFIER_BASE + i, Scope::External)` for `i ∈ 0..(5 - n_real)`, where `PADDING_DIVERSIFIER_BASE = 1000` is a named constant in `builder.rs`. Orchard's FF1-based diversifier permutation hides the index space from outside observers, so the literal value is observationally indistinguishable from any other choice — no soundness or privacy primitive depends on it. The constant exists so that the builder and the per-slot tests share a single anchor (preventing drift), and so that `PrecomputedRandomness` captured in Phase 1 re-derives the same addresses in Phase 2. `Scope::External` is hardcoded because padding slots always set `is_internal = false`, which fixes condition 11's `selected_ivk` mux to the external `ivk`. Internal-scope padding would also need to mux the diversifier derivation path and is unused — internal-scope addresses in standard wallets are reserved for change/self-receive.
